...
# import tensorflow.contrib.slim.nets as nets # perceptual loss function (plf)
# vgg = nets.vgg # plf
...
class discriminator_reward():
    ...
    def __init__(self,filepath=None):
        if filepath == None:
            filepath = '/home_local/grace/20180808_0129/5000.ckpt'
    ...
    def __call__(self, im):
        ...
        im = np.expand_dims(np.abs(im), axis=0) # expected input[1, 256, 256, 2] to have 2 channels, but got 256 channels instead
        im = np.transpose(im, (0, 3, 1, 2))
        output = torch.Tensor(im).cuda()
        im = output
...
class recon_env():
 ...
 # Every step is we sampled data once
  def step(self, action):
    ...
    self.writer.add_image('input', mag_input)

    torchvision.utils.save_image(mag_input, "./mag_input.png") # comment to change reward
    #endgrace
    ...
    curr_reward = self.reward(self.curr_img-self.Y_Y)/self.base_reward # original
    ...
    torchvision.utils.save_image(output, "./output.png") # comment to change reward
    ## end grace

    # grace plf (uncomment original for l2 reward)
    # im0 = util.load_image("./output.png").transpose(2, 0, 1) / 255.
    # im1 = util.load_image("./mag_input.png").transpose(2, 0, 1) / 255.
    # pred = Variable(torch.FloatTensor(im0)[None, :, :, :])  # util.im2tensor(im0) # RGB image from [-1,1]
    # ref = Variable(torch.FloatTensor(im1)[None, :, :, :])  # util.im2tensor(im1)
    # dist = loss_fn.forward(pred, ref, normalize=True).item()
    # curr_reward = dist # / base_reward ## if we normalize it we'll divide by 0 lmfao
    # end grace plf
