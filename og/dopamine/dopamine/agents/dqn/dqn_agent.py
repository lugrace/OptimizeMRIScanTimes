...
OBSERVATION_SHAPE = 256  # Size of a downscaled Atari 2600 frame.
STACK_SIZE = 2  # Number of frames in the state stack.
...
class DQNAgent(object):
  ...
  def _build_networks(self):
    ...
    self._q_argmax_all = tf.nn.top_k(self._net_outputs.q_values,k=OBSERVATION_SHAPE).indices[0]
  ...
  def _select_action(self):
    ...
    epsilon = self.epsilon_eval if self.eval_mode else self.epsilon_fn(
        ...
        self.epsilon_train)
    sampled_lines = self.env.get_sampled_lines()
    if random.random() <= epsilon:
      # Choose a random action with probability epsilon.
      #return random.randint(0, self.num_actions - 1)
      candidates = np.random.permutation(self.num_actions)
    else:
      # Choose the action with highest Q-value at the current state.
      #return self._sess.run(self._q_argmax, {self.state_ph: self.state})
      candidates = self._sess.run(self._q_argmax_all, {self.state_ph: self.state})
    for candidate in candidates:
      if sampled_lines[candidate] == 0:
        return candidate
