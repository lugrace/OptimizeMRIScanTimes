...
#import gym
#from dopamine.recon_env import val_recon_env as recon_env
from dopamine.recon_env import recon_env
import numpy as np
...
def create_atari_environment(game_name, sticky_actions=True, base_dir=None):
  ...
  funcs = game_name.split(';')
  print(funcs)
  recon_func = getattr(recon_env,funcs[0])()
  reward_func = getattr(recon_env,funcs[1])()
  R = int(funcs[2])
  #recon_func = recon_env.cs_recon()
  #reward_func = recon_env.L2_reward()
  env = recon_env.recon_env(recon_func, reward_func, base_dir, R)
  return env
...
class Runner(object):
  ...
  def __init__(self,
    ...
    self._environment = create_environment_fn(game_name, sticky_actions, base_dir)
    # Set up a session and initialize variables.
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    config.allow_soft_placement=True
    # config.log_device_placement=True # grace added, see which device is used
